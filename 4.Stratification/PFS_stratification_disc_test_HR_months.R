...
for (i in 1:18) {
  ...
  cox.train <- summary(coxph(Surv(f.train$PFS, f.train$PFS_Status) ~ pt.stratify.train))
  cox.test <- summary(coxph(Surv(f.test$PFS, f.test$PFS_Status) ~ pt.stratify.test))
...
t <- 60
f.train$PFS_c <- ifelse(f.train$PFS > t, t, f.train$PFS)
f.train$PFS_Status_c <- ifelse(f.train$PFS > t, 0, f.train$PFS_Status)

pt.stratify.train <- factor(f.train$Risk_Prediction >= cutoff_value)
f.train$DeepFeatures <- factor(pt.stratify.train, levels = c("TRUE", "FALSE"))

cox_train <- coxph(Surv(PFS_c, PFS_Status_c) ~ pt.stratify.train, data = f.train)
...
fit <- survfit(Surv(PFS_c, PFS_Status_c) ~ DeepFeatures, data = f.train)
...
f.test$PFS_c <- ifelse(f.test$PFS > t, t, f.test$PFS)
f.test$PFS_Status_c <- ifelse(f.test$PFS > t, 0, f.test$PFS_Status)
...
cox_test <- coxph(Surv(PFS_c, PFS_Status_c) ~ pt.stratify.test, data = f.test)
...
fit <- survfit(Surv(PFS_c, PFS_Status_c) ~ DeepFeatures, data = f.test)
